...
data {
  ...
  real<lower=0> Z[T];             // Incidence number
  int population;                 // Population size (constant)
  real max_Y;                     // Upper bound on infected density
  /* Hyperparameters */
  ...
  real<lower=0> loc_gamma;        // Control location of prior on recovery rate
...
generated quantities {
  ...
  z_tilde = get_z_tilde(T, y0, all_rates, ts, args_real, args_int, population);
}
