...
# -*- coding: utf-8 -*-
"""
Produce the results of Figure 2 (main text).

The tested time-series describe simulated epidemics on a clustered
network.
...
ts_dir = '../time_series/clustered_networks/'
res_dir = '../results/clustered_networks/'
...
for ts_file, loc_gamma in [('independent', 6.45),
                           ('interacting', 5.90)]:
    # Read data as CSV
    ...
    with open(join(res_dir, 'prevalence_' + ts_file + '.pck'), 'wb') as f:
        pickle.dump(fit, f)
    # Fit with incidence model and dump results to disk
    fit = incidence_model.sampling(data_incidence,
                                   iter=2000, chains=4,
                                   control={'max_treedepth': 15})
    with open(join(res_dir, 'incidence_' + ts_file + '.pck'), 'wb') as f:
        pickle.dump(fit, f)
